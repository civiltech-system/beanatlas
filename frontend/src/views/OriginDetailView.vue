  ...
  <div class="max-w-3xl mx-auto px-6 py-12">
    ...
    <template v-else-if="origin">
      ...
      <div class="mb-8">
        ...
        <h1 class="text-4xl font-serif font-bold text-coffee-600">{{ locale === 'ja' ? origin.country_ja : origin.country }}</h1>
        <p class="text-coffee-400 mt-2">{{ origin.altitude_min }}–{{ origin.altitude_max }}m · {{ origin.climate }}</p>
      </div>

      <!-- Flavor Profile -->
      <div class="bg-white rounded-2xl p-6 shadow-sm mb-6">
        <h2 class="text-lg font-semibold text-coffee-600 mb-4">{{ t('detail.flavorProfile') }}</h2>
        <div class="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div class="space-y-3 flex-1 w-full">
            <FlavorIndicator :label="t('flavor.acidity')" :value="origin.acidity" />
            <FlavorIndicator :label="t('flavor.bitterness')" :value="origin.bitterness" />
            <FlavorIndicator :label="t('flavor.sweetness')" :value="origin.sweetness" />
            <FlavorIndicator :label="t('flavor.body')" :value="origin.body" />
          </div>
        </div>
      </div>
      ...
      <!-- Flavor Notes -->
      <div class="mb-6">
        <h2 class="text-lg font-semibold text-coffee-600 mb-3">{{ t('detail.flavorNotes') }}</h2>
      ...
      <div class="prose prose-coffee max-w-none mb-8">
        <p class="text-coffee-600 leading-relaxed">{{ locale === 'ja' ? origin.description_ja : origin.description }}</p>
      </div>
